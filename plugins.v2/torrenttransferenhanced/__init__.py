...
class TorrentTransferEnhanced(_PluginBase):
    # 插件名称
    plugin_name = "转移做种增强"
    # 插件描述
    plugin_desc = "支持暂停和未完成的种子定时转移做种。"
    # 插件图标
    plugin_icon = "https://raw.githubusercontent.com/cancelpt/MoviePilot-Plugins/refs/heads/main/icons/fruit.png"
    # 插件版本
    plugin_version = "0.1"
    # 插件作者
    plugin_author = "jxxghp,cancelpt"
    # 作者主页
    author_url = "https://github.com/cancelpt"
    # 插件配置项ID前缀
    plugin_config_prefix = "enhanced_torrenttransfer_"
    # 加载顺序
    plugin_order = 18
    # 可使用的用户级别
    auth_level = 2
    ...
    def init_plugin(self, config: dict = None):
        ...
        if config:
            ...
            self._forcetrans = config.get("forcetrans")
            ...
            self._torrent_tags = self._add_torrent_tags.strip().split(
                ",") if self._add_torrent_tags else []
        ...
        if self.get_state() or self._onlyonce:
            ...
            if self._autostart:
                ...
                self._scheduler.add_job(
                    self.check_recheck, 'interval', minutes=0.5)
    ...
    def __validate_config(self) -> bool:
        ...
        if self._fromtorrentpath and not Path(self._fromtorrentpath).exists():
            ...
            self.systemmessage.put(
                f"源下载器种子文件保存路径不存在：{self._fromtorrentpath}", title="自动转移做种")
            return False
    ...
    def transfer(self):
        ...
        for torrent in torrents:
            ...
            # 种子为无标签,则进行规范化
            is_torrent_labels_empty = torrent_labels == [
                ''] or torrent_labels == [] or torrent_labels is None
            if is_torrent_labels_empty:
                torrent_labels = []
            
            # 排除既无视进度转移的标签，又没有完成种子
            progress = self.__get_progress(torrent, from_service.type)
            if self._forcetrans:
                is_force = False
                for label in self._forcetrans.split(','):
                    if label in torrent_labels:
                        is_force = True
                        break
                if not is_force and progress < 1:
                    logger.info(f"种子 {hash_str} 既不含有无视进度转移的标签，又没有完成种子，跳过 ...")
                    continue
        ...
        if trans_torrents:
            ...
            for torrent_item in trans_torrents:
                # 检查种子文件是否存在
                torrent_file = Path(self._fromtorrentpath) / \
                ...
                if torrent_info:
                    ...
                    if self._deleteduplicate:
                        ...
                        to_downloader.delete_torrents(delete_file=False, ids=[
                                                      torrent_item.get('hash')])
                        del_dup += 1
                    else:
                        logger.info(
                ...
                if self.downloader_helper.is_downloader("qbittorrent", service=from_service):
                    ...
                    if not main_announce:
                        ...
                        # 读取fastresume文件
                        fastresume_file = Path(
                            self._fromtorrentpath) / f"{torrent_item.get('hash')}.fastresume"
                        ...
                        try:
                            ...
                            # 读取trackers
                            fastresume_trackers = torrent_fastresume.get(
                                'trackers')
                            if isinstance(fastresume_trackers, list) \
                                ...
                                # 替换种子文件路径
                                torrent_file = settings.TEMP_PATH / \
                                    f"{torrent_item.get('hash')}.torrent"
                                # 编码并保存到临时文件
                ...
                if not download_id:
                    ...
                    if self.downloader_helper.is_downloader("qbittorrent", service=to_service):
                        if self._skipverify:
                            if self._autostart:
                                logger.info(
                                    f"{download_id} 跳过校验，开启自动开始，注意观察种子的完整性")
                                self.__add_recheck_torrents(
                                    to_service, download_id)
                            else:
                                # 跳过校验
                                logger.info(
                                    f"{download_id} 跳过校验，请自行检查手动开始任务...")
                        else:
    ...
    @staticmethod
    def __get_progress(torrent: Any, dl_type: str) -> float:
        """
        获取种子进度
        """
        try:
            return torrent.get("progress") if dl_type == "qbittorrent" else torrent.percentDone
        except Exception as e:
            print(str(e))
            return 0
